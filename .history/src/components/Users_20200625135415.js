                ...
                .map(n => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="right" style={{color:"#ffffff"}}>{n.pk_id}</TableCell>
                      <TableCell align="right" style={{color:"#ffffff"}}>{n.pk_id_norm}</TableCell>
...
componentDidMount() {
    axios.get(`http://localhost:8080/Summer_Internship_Backend/DisplayDataServlet`)
    .then((response) => {
      console.log(response);
      this.setState({ data: response.data });
    })
...
handle_predict= (event) => {
  ...
  {headers: { 'Content-Type': 'application/json' }}
    ...
    (response) => { 
        ...
        this.setState({prediction : response.data.predictions})
 ...
 render() {
  ...
  <div className={classes.table_Wrapper}>
        <Table  autoid="invoice-table-collector" className={classes.table} aria-labelledby="tableTitle">
        <EnhancedTableHead
            numSelected={selected.length}
            ...
            {data
              ...
              if (prediction.length>0){
                for(var index=0;index<prediction.length;index++){
                  if(prediction[index].pk_id === n.pk_id){
                    pay_type=prediction[index].payment_Type;
                    pay_part=prediction[index].payment_Amount;
                  }
                }  
              ...
              {emptyR > 0 && (
                <TableRow style={{ height: 20 * emptyn }}>
                  <TableCell colSpan={5} />
                </TableRow>
              )}
            </TableBody>
